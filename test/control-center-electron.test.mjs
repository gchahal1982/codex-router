...
test("preload constructs exact positional IPC payloads", async () => {
  ...
  const cases = [
    ...
    ["getChatGptAccounts", [], null],
    ["addChatGptAccount", ["Work subscription", true], { label: "Work subscription", preferred: true }],
    ["renameChatGptAccount", ["default", "Work Pro"], { accountId: "default", label: "Work Pro" }],
    ["setChatGptAccountPurpose", ["default", "auraone"], { accountId: "default", purpose: "auraone" }],
    ["setPreferredChatGptAccount", ["chatgpt_abcdefghijklmnop"], { accountId: "chatgpt_abcdefghijklmnop" }],
    ["setChatGptAccountPaused", ["chatgpt_abcdefghijklmnop", true], { accountId: "chatgpt_abcdefghijklmnop", paused: true }],
    ["removeChatGptAccount", ["chatgpt_abcdefghijklmnop"], { accountId: "chatgpt_abcdefghijklmnop" }],
    ["refreshChatGptAccount", ["chatgpt_abcdefghijklmnop"], { accountId: "chatgpt_abcdefghijklmnop" }],
    ["loginChatGptAccount", ["chatgpt_abcdefghijklmnop"], { accountId: "chatgpt_abcdefghijklmnop" }],
...
test("settings exposes explicit chat and scheduled-task model defaults", async () => {
  ...
  // The reserve allowance is chosen by the Codex app, never assigned in a
  // setting, so it must not appear as a selectable default.
  assert.doesNotMatch(settings, /slug: "gpt-reserve"/);
  assert.match(settings, /settings\.takeover\.title/);
  assert.match(settings, /setNativeTakeoverModel\(/);
  assert.match(settings, /setNativeTakeoverEffort\(/);
  assert.match(settings, /setNativeTakeoverCronEffort\(/);
