...
enum RouterChineseText {
  static let values: [String: String] = [
    ...
    "foundation": "基金会",
    "reserve": "备用",
    "Paused accounts stay out of rotation": "已暂停的账户不参与轮换",
    "Home login stays on": "当前登录保持开启",
    "Turn this subscription on": "打开此订阅",
    "Turn this subscription off": "关闭此订阅",
    "Preferred subscription": "首选订阅",
    "Click to prefer this subscription": "点按以设为首选订阅",
    "Double-click to prefer this subscription": "点按两下以设为首选订阅",
    "Use a banked reset on %@ now": "立即使用 %@ 的已存重置额度",
    "Use a banked reset on %@": "在 %@ 上使用已存重置额度",
    "Used a banked reset on %@.": "已在 %@ 上使用一个已存重置额度。",
    "%@ has no banked reset left to use.": "%@ 没有可用的已存重置额度。",
    "%@ is not rate limited, so no reset was used.": "%@ 未受速率限制，因此未使用重置额度。",
    "That banked reset was already used on %@.": "该已存重置额度已在 %@ 上使用过。",
    "The banked reset for %@ could not be used.": "无法使用 %@ 的已存重置额度。",
